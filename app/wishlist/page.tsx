...
import { Heart, Trash2, ShoppingBag, ArrowRight, Sparkles, RefreshCw } from "lucide-react";
...
export default function WishlistPage() {
  ...
  const { items, removeItem, clearWishlist, isLoading, forceRefresh } = useWishlistStore();
      ...
      <div className="relative overflow-hidden"
        ...
        {[...Array(5)].map((_, i) => (
          <div key={i} className="absolute pointer-events-none"
            style={{ left: `${15 + i * 18}%`, top: `${20 + (i % 3) * 22}%`,
              opacity: 0.06 + i * 0.03, fontSize: `${14 + (i % 3) * 8}px`,
              animation: `float ${3 + i * 0.5}s ease-in-out infinite alternate`, color: "#C8A45D" }}>
            ♥
          </div>
        ))}
        ...
        <div className="relative z-10 flex flex-col items-center justify-center text-center px-6" style={{ minHeight: "280px" }}>
          ...
          <Heart size={28} fill="rgba(200,164,93,0.3)" style={{ color: "#C8A45D", marginBottom: "16px" }} />
      ...
      <div className="max-w-4xl mx-auto px-4 py-10">
        ...
        {!isLoading && items.length === 0 && (
          <div className="flex flex-col items-center justify-center py-24 text-center rounded-3xl"
            ...
            <h2 className="font-serif text-3xl mb-3" style={{ color: "#1A0A05" }}>Nothing saved yet</h2>
            ...
            <Link href="/products"
              ...
              style={{ background: "#1A0A05", color: "#C8A45D", border: "1px solid #1A0A05" }}
        ...
        {items.length > 0 && (
          <>
            <div className="flex items-center justify-between mb-8">
              <div>
                <h2 className="font-serif text-2xl" style={{ color: "#1A0A05" }}>Saved Pieces</h2>
            ...
            <div className="space-y-4">
              {items.map((item, idx) => (
                <div key={item.id}
                  className="group flex items-center gap-5 rounded-2xl p-4"
                  style={{
                    ...
                    <div className="relative rounded-xl overflow-hidden"
                      ...
                      {!item.inStock && (
                        <div className="absolute inset-0 flex items-center justify-center"
                          ...
                          <span className="text-[8px] font-bold uppercase tracking-wider px-2 py-1 rounded"
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-xs mt-1" style={{ color: "rgba(26,10,5,0.4)" }}>{item.weight}g</p>
                  </div>
                  ...
                  <div className="flex flex-col items-end gap-3 flex-shrink-0">
                    {item.inStock ? (
                      <Link href={`/products/${item.id}`}
                        ...
                        onMouseLeave={e => { e.currentTarget.style.background = "#1A0A05"; e.currentTarget.style.color = "#C8A45D"; }}
                      >
                        <ShoppingBag size={11} /> Enquire
                      </Link>
                    ) : (
                      <span className="px-4 py-2 rounded-full text-[10px] font-bold uppercase tracking-wider"
                        style={{ border: "1px solid rgba(181,138,61,0.2)", color: "rgba(181,138,61,0.5)" }}>
                        Unavailable
                      </span>
                    )}
                    <button onClick={() => handleRemove(item.id)} disabled={isLoading}
                      ...
                      onMouseLeave={e => { e.currentTarget.style.color = "rgba(26,10,5,0.25)"; e.currentTarget.style.background = "transparent"; }}
            ...
            {/* Footer CTA */}
            <div className="mt-10 flex flex-col sm:flex-row items-center justify-between gap-4 p-6 rounded-2xl"
              style={{ background: "linear-gradient(135deg, #1A0A05, #2D0A0A)", border: "1px solid rgba(200,164,93,0.2)" }}>
              <div>
                <p className="font-serif text-lg" style={{ color: "#FDFBF7" }}>Discover More Treasures</p>
                <p className="text-xs mt-1" style={{ color: "rgba(253,251,247,0.4)" }}>Explore our full collection</p>
              </div>
              <Link href="/products"
                ...
                style={{ background: "rgba(200,164,93,0.15)", color: "#C8A45D", border: "1px solid rgba(200,164,93,0.3)" }}
                onMouseEnter={e => { e.currentTarget.style.background = "#C8A45D"; e.currentTarget.style.color = "#1A0A05"; }}
                onMouseLeave={e => { e.currentTarget.style.background = "rgba(200,164,93,0.15)"; e.currentTarget.style.color = "#C8A45D"; }}
              >
