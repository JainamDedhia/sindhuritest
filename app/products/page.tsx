      ...
      <span className="flex h-5 w-5 items-center justify-center rounded-full bg-[#C8A45D] text-[10px] text-white font-bold">
          ...
          {activeFiltersCount > 0 && (
            <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: "auto" }} exit={{ opacity: 0, height: 0 }} className="mb-8 overflow-hidden">
              <div className="flex flex-wrap items-center justify-center gap-2">
                ...
                <button onClick={clearFilters} className="flex items-center gap-1.5 px-3 py-1.5 text-[10px] font-bold uppercase tracking-widest text-gray-400 hover:text-red-500 hover:bg-red-50 rounded-full transition-all">
        ...
        {loading ? (
          ...
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}
            ...
            <p className="text-sm text-gray-500 mb-6 max-w-xs">Try adjusting your filters or explore a different collection</p>
            <button onClick={clearFilters} className="rounded-full bg-[#1A0A05] px-8 py-3 text-sm font-medium text-white hover:bg-[#2A1505] transition-all">
              View All Collection
            </button>
          </motion.div>
        ) : (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 0.4 }}
            ...
            {filteredProducts.map((item, i) => (
              <motion.div key={item.id} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: Math.min(i * 0.05, 0.3) }}>
                <ProductCard
                  product={{
                    id: item.id,
                    title: item.name,
        ...
        {isFilterOpen && (
          <>
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
              className="fixed inset-0 z-50 bg-black/50 backdrop-blur-sm" onClick={() => setIsFilterOpen(false)} />
            ...
            <motion.div initial={{ x: "100%" }} animate={{ x: 0 }} exit={{ x: "100%" }}
              ...
              <div className="bg-[#1A0A05] px-6 py-5 flex items-center justify-between">
                <div>
                  ...
                  <p className="text-xs text-gray-400 mt-0.5">{filteredProducts.length} pieces found</p>
                </div>
                <button onClick={() => setIsFilterOpen(false)} className="p-2 rounded-full bg-white/10 text-white hover:bg-white/20 transition-colors">
                  <X size={18} />
                </button>
              </div>
              ...
              <div className="flex-1 overflow-y-auto p-6 space-y-8 pb-28">
                    ...
                    {[
                      ...
                      <button key={opt.val} onClick={() => setSortBy(opt.val as any)}
                        className={`w-full flex items-center justify-between px-4 py-3 rounded-lg text-sm font-medium text-left transition-all ${
                          sortBy === opt.val ? 'bg-[#C8A45D]/10 text-[#8B6914] border border-[#C8A45D]/30' : 'text-gray-600 hover:bg-gray-50 border border-transparent'
                        }`}>
                        {opt.label}
                ...
                <div>
                  <div className="flex justify-between items-center mb-4">
                    ...
                    {selectedCategories.length > 0 && (
                      <button onClick={() => setSelectedCategories([])} className="text-xs font-medium text-gray-400 hover:text-red-500 transition-colors">Reset</button>
                    )}
                  </div>
                  <div className="space-y-1">
                      ...
                      return (
                        <label key={category.id} className="flex items-center justify-between p-3 rounded-lg hover:bg-gray-50 cursor-pointer group transition-colors">
                          <div className="flex items-center gap-3">
                            <div className={`w-5 h-5 rounded border-2 flex items-center justify-center transition-all ${isSelected ? 'bg-[#C8A45D] border-[#C8A45D]' : 'border-gray-300 bg-white'}`}>
                              {isSelected && <Check size={11} className="text-white" strokeWidth={3} />}
                            </div>
                            <span className={`text-sm ${isSelected ? 'font-semibold text-gray-900' : 'text-gray-600'}`}>{category.name}</span>
                          </div>
                          <span className="text-[10px] text-gray-400 bg-gray-100 px-2 py-0.5 rounded-full">{count}</span>
                          <input type="checkbox" className="hidden" checked={isSelected} onChange={() => toggleCategory(category.name)} />
                        </label>
                      );
                    })}
                  </div>
                ...
                <div>
                  ...
                  <div className="space-y-1">
                    {[{ val: 'all', label: 'All Pieces' }, { val: 'inStock', label: 'In Stock Only' }].map((opt) => (
                      <label key={opt.val} className="flex items-center gap-3 p-3 rounded-lg hover:bg-gray-50 cursor-pointer">
                        <div className={`w-5 h-5 rounded-full border-2 flex items-center justify-center transition-all ${stockFilter === opt.val ? 'border-[#C8A45D]' : 'border-gray-300'}`}>
                          {stockFilter === opt.val && <div className="w-2.5 h-2.5 rounded-full bg-[#C8A45D]" />}
                        </div>
                        <span className={`text-sm ${stockFilter === opt.val ? 'font-semibold text-gray-900' : 'text-gray-600'}`}>{opt.label}</span>
                        <input type="radio" className="hidden" checked={stockFilter === opt.val} onChange={() => setStockFilter(opt.val as any)} />
                      </label>
                    ))}
                  </div>
                </div>
              ...
              <div className="absolute bottom-0 w-full p-5 bg-white border-t border-gray-100">
                <div className="flex gap-3">
                  <button onClick={clearFilters} className="flex-1 py-3 rounded-xl border-2 border-gray-200 text-sm font-semibold text-gray-700 hover:border-gray-300 transition-all">
                    Clear All
                  </button>
                  <button onClick={() => setIsFilterOpen(false)} className="flex-1 py-3 rounded-xl bg-[#1A0A05] text-sm font-semibold text-white hover:bg-[#2A1505] transition-all shadow-lg">
