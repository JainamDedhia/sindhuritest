  ...
  return (
    <section className="py-12 md:py-20 bg-[#FDFBF7] relative">
      ...
      <div className="container mx-auto max-w-275 px-5">
        ...
        <div className="grid grid-cols-2 lg:grid-cols-3 gap-3 md:gap-5 h-auto lg:h-120">
          
          {/* 🔥 CARD 1 - HERO (FIXED) */}
          {items[0] && (
            <Link
              href={getCategoryFilterUrl(items[0].title)} // 🔥 FIXED
              className="group relative col-span-2 rounded-xl overflow-hidden cursor-pointer h-80 lg:h-full shadow-sm hover:shadow-2xl transition-all duration-700"
            >
              <div className="absolute inset-0 transition-transform duration-1000 ease-out group-hover:scale-105">
                <img src={items[0].image_url} alt={items[0].title} className="h-full w-full object-cover" />
                <div className="absolute inset-0 bg-linear-to-t from-[#2A0A0A] via-[#2A0A0A]/60 to-transparent opacity-95" />
              </div>
          ...
          <div className="col-span-2 lg:col-span-1 grid grid-cols-2 lg:grid-cols-1 gap-3 md:gap-5 h-auto lg:h-full">
            ...
            {items[1] && (
              <Link
                ...
                className="group relative w-full h-50 lg:h-full rounded-xl overflow-hidden shadow-sm hover:shadow-lg transition-all"
              >
                <div className="absolute inset-0 transition-transform duration-700 group-hover:scale-105">
                  <img src={items[1].image_url} alt={items[1].title} className="h-full w-full object-cover" />
                  <div className="absolute inset-0 bg-linear-to-t from-[#660707] via-[#062C1D]/50 to-transparent opacity-90" />
                </div>
            ...
            {items[2] && (
              ...
              >
                <div className="absolute inset-0 transition-transform duration-700 group-hover:scale-105">
                  ...
                  <div className="absolute inset-0 bg-linear-to-t from-[#660707] via-[#120F2D]/50 to-transparent opacity-90" />
                </div>
